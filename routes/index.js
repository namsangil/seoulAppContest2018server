module.exports = function(app, Event, Reply, Mem, Feel, Ecounter, SearchRecommend)
{   //파일첨부 하던중
   ...
   // }); 
    ...
    app.get('/api/banner', function(req, res){
     	  	fs.readFile('./public/banner.html', 'utf-8', function(error, data) {
   		console.log('%s',data);
	 	res.json(eval('('+data+')'));
	   }); 
    	}
    ...
    app.get('/api/searchRecommend', function(req,res){
	SearchRecommend.find({}).exec(function(err, keywords){
		console.log(err);
    ...
    app.get('/api/eventsAll', function(req,res){
        ...
        if(condition == 1){//인기순 조회
            Event.find({}).sort({like_count: 'desc'}).exec(function(err, events){
	...
	event.date = req.body.date || req.params.date || req.query.date;
	event.startTime = req.body.startTime || req.params.startTime || req.query.startTime;
	event.endTime = req.body.endTime || req.params.endTime || req.query.endTime;
	...
	event.host = req.body.host || req.params.host || req.query.host;
	var tmpPlace = req.body.place || req.params.place || req.query.place; 
	event.place = eval(tmpPlace);
    ...
    // CREATE(INSERT) REPLY
     app.use('/api/events/reply/write', function(req, res){
        ...
        reply.device_id = req.body.device_id || req.params.device_id || req.query.device_id;        
        reply.text = req.body.text || req.params.text || req.query.text;            
    ...
    app.use('/api/events/feel', function(req, res){
            ...
            var val1; // event의 count 변화 값: +1/-1
        
            // 클릭 시, 기존의 감정 상태의 토글값을 얻음
            if (yn == true || yn == "1" || yn == 1) { 
                yn = false; 
                val1 = -1; }
            else { 
                yn = true; 
